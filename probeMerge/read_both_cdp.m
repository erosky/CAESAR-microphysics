function data=read_cdp(ncfile)
    ...
    %Rebin outer CDP
    endbins2 = ncreadatt(ncfile, 'CCDP_LWO', 'CellSizes')
    endbins2 = squeeze(endbins2);
    midbins2 = (endbins2(2:end) + endbins2(1:end-1))./2;

    old_conc2 = ncread(ncfile,'CCDP_LWO');
    old_conc2 = permute(squeeze(old_conc2),[2 1]);
    conc2 = zeros(size(old_conc2));
    
    for i = 1:length(midbins2)
        old_width = endbins2(i+1) - endbins2(i);

        for j = 1:length(data.midbins)
            overlap = min(endbins2(i+1), data.endbins(j+1)) - ...
                max(endbins2(i), data.endbins(j));

            if overlap > 0
                fraction = overlap / old_width;
                conc2(:,j) = conc2(:,j) + old_conc2(:,i) * fraction;
            end
        end
    end

    %Convert cdp conc to #/m4
    conc1 = ncread(ncfile,'CCDP_LWI');
    conc1 = permute(squeeze(conc1),[2 1]);
    bw = (data.endbins(2:end) - data.endbins(1:end-1))*10^(-6); %convert to m
    data.conc = (((conc1+conc2)/2)*10^(6))./bw; %should now be units of #/m4
